...
- **Modelo de Domínio Rico (Rich Domain Model) é obrigatório**: favorecer a inclusão de comportamento (validações, cálculos, transições de estado) diretamente nas entidades e Value Objects. Evitar o **Modelo de Domínio Anêmico**, onde entidades são apenas contêineres de dados com getters e setters.
