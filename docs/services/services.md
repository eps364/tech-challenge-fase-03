...
| Keycloak (Realm `tech-challenge`) | http://localhost:8000/realms/tech-challenge/account/ | admin, user, cliente, dono, gerente | admin, user, cliente, dono, gerente | Usuários importados de `docker/keycloak/realm/realm-export.json` (cada usuário usa a mesma senha do próprio nome). |
...
| Auth Service | via API Gateway (`http://localhost:8761`) | Token JWT (Keycloak) | Token JWT (Keycloak) | Serviço de autenticação registrado no Eureka; sem porta exposta no host no `compose.yml`. |
